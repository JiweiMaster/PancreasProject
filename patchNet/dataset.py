...
class PatchDataset(object):
    def __init__(self,trains,souts,couts,preds,gts,dilation_s=0,dilation_c=0):
        self.trains = trains
        self.souts = souts
        self.couts = couts
        self.preds = preds
        ...
        # size大小
        self.sz = souts.shape[0]

    def __len__(self):
        return self.trains.shape[0]

    def __getitem__(self, index):
        np.random.seed()
        h, w, mod = self.trains[index].shape
        dimg = Image.fromarray(self.trains[index])

        l = self.gts[index].copy()
        sin = self.souts[index].copy()
        cin = self.couts[index].copy()
        
        oripredmask_ori = self.preds[index].copy()
